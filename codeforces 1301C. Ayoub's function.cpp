...
int main() {
    ...
    while (tc--) {
        ...
        ll sections = m + 1;
        ll low = dif / sections;
        ll high = (dif + sections - 1)/sections;
        ll highCnt = dif % sections;
        ll lowCnt = sections - highCnt;
        ll badPair = low * (low + 1) / 2 * lowCnt + high * (high + 1) / 2 * highCnt;
        ll allPair = n * (n + 1)/2;
        cout << allPair - badPair << endl;
    }
