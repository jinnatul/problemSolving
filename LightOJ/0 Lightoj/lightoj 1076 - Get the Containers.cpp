...
bool call(ll mid) {
    ll sum = 0, pos = 0, cnt = 1;

    while (pos < vc.size()) {
        if (vc[pos] > mid) return false;
        else if (vc[pos] + sum <= mid) {
            sum += vc[pos];
            pos++;
        } else {
            if (cnt >= numOfContainer && pos < vc.size()) return false;
            sum = 0;
            cnt++;
        }
    }
...
int main() {
    ...
    sc1(tc);
    while (tc--) {
        sc2(num, numOfContainer);
        foR0(num) {
            sc1(n);
            vc.pb(n);
        }

        ll hi = mod, lw = 0, ans = -1;
        while (lw <= hi) {
            ll mid = (lw + hi) >> 1;
            if (call(mid)) {
                ans = mid;
