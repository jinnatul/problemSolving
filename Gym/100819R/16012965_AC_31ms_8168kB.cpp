...
bool xx(const student& stua, const student& stub)
{
    int a=strcmp(stua.bb,stub.bb);
    if(a==0)
    {
        int b=strcmp(stua.aa,stub.aa);
        return (b<0);
    }
    return (a<0);
}
...
int main()
{
    FastIO;
    //sieve();
    int n,m,i,j=0,k,ck=0,dk=0,a,b;

    scanf("%d", &n);

    for(i=0; i<n; i++){
       scanf("%s %s",stu[i].aa,stu[i].bb);
    }
    sort(stu, stu+n, xx);
