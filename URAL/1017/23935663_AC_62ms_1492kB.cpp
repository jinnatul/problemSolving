...
ll dp[size1][size1], rows;

ll call (ll lastHigh, ll have) {
    if (have == 0) return 1;
    if(have <= lastHigh) return 0;

    if (dp[lastHigh][have]) return dp[lastHigh][have];
    ll sum = 0;
    for (ll i = lastHigh + 1; i <= have; ++i) {
        sum += call(i, have - i);
    }

    return dp[lastHigh][have] = sum;
}
