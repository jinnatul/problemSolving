...
int main()
{
	...
	v.pb({v[1].ff,v[0].ss});
	v.pb({v[0].ff,v[1].ss});
	if(x>=v[0].ff && x<=v[2].ff){
		ans=min(abs(y-v[0].ss),abs(y-v[1].ss));
	}
	else if(y>=v[0].ss && y<=v[3].ss){
		ans=min(abs(x-v[0].ff),abs(x-v[2].ff));
	}
	else{
		ans=min(ans,sqrt(((x-v[0].ff)*(x-v[0].ff))+(y-v[0].ss)*(y-v[0].ss)));
		ans=min(ans,sqrt(((x-v[1].ff)*(x-v[1].ff))+(y-v[1].ss)*(y-v[1].ss)));
		ans=min(ans,sqrt(((x-v[2].ff)*(x-v[2].ff))+(y-v[2].ss)*(y-v[2].ss)));
		ans=min(ans,sqrt(((x-v[3].ff)*(x-v[3].ff))+(y-v[3].ss)*(y-v[3].ss)));
	}
	pf("%0.3lf\n", ans);
	#ifdef VAMP
