...
ll dp[size1][size1], total, arr[size1];
ll call(ll nDigitm, ll num) {
    if(nDigitm == 1) return 1;

    if(dp[nDigitm][num]) return dp[nDigitm][num];

    ll cnt = 0;
    for (ll i = 1; i <= total; ++i) {
        if(abs(arr[num] - arr[i]) <= 2)
            cnt += call(nDigitm - 1, i);
    }

    return dp[nDigitm][num] = cnt;
}
